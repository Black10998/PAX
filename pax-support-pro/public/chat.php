...
function pax_sup_enqueue_public_assets() {
    ...
    wp_enqueue_style(
        'pax-google-fonts',
        'https://fonts.googleapis.com/css2?family=Orbitron:wght@400;500;600;700&family=Tajawal:wght@300;400;500;700&display=swap',
        array(),
        null
    );
    ...
    // ملفات الستايل
    wp_enqueue_style(
        'pax-livechat-unified',
        PAX_SUP_URL . 'public/css/livechat-unified.css',
        array( 'pax-css-variables', 'pax-google-fonts' ),
        PAX_SUP_VER
    );

    wp_enqueue_style(
        'pax-live-widget',
        PAX_SUP_URL . 'assets/css/pax-live-widget.css',
        array( 'pax-livechat-unified' ),
        PAX_SUP_VER
    );

    wp_enqueue_style(
        'pax-support-pro',
        PAX_SUP_URL . 'public/assets.css',
        array( 'dashicons', 'pax-google-fonts', 'pax-livechat-unified' ),
        PAX_SUP_VER
    );
    ...
    wp_enqueue_script(
        'pax-unified-chat',
        PAX_SUP_URL . 'assets/js/pax-unified-chat.js',
        array( 'pax-support-pro' ),
        PAX_SUP_VER,
        true
    );

    $position       = $options['launcher_position'] ?? 'bottom-right';
    ...
    $css  = ':root{';
    $css .= '--pax-bg:' . esc_html( $options['color_bg'] ?? '#0b1020' ) . ';';
    $css .= '--pax-panel:' . esc_html( $options['color_panel'] ?? '#101626' ) . ';';
    $css .= '--pax-border:' . esc_html( $options['color_border'] ?? '#1f2937' ) . ';';
    $css .= '--pax-text:' . esc_html( $options['color_text'] ?? '#f9fafb' ) . ';';
    $css .= '--pax-sub:' . esc_html( $options['color_sub'] ?? '#9ca3af' ) . ';';
    $css .= '--pax-accent:' . esc_html( $options['color_accent'] ?? '#6366f1' ) . ';';
    $css .= '--pax-reaction-bg:rgba(' . implode( ',', $reaction_rgb ) . ',0.9);';
    $css .= '--pax-reaction-bg-hover:' . esc_html( $reaction_color ) . ';';
    $css .= '--pax-reaction-border:rgba(' . implode( ',', $reaction_rgb ) . ',0.5);';
    $css .= '--pax-reaction-border-hover:rgba(' . implode( ',', $reaction_rgb ) . ',0.7);';
    ...
    $localize = array(
        'options'  => array(
            ...
            'color_accent'         => $options['color_accent'] ?? '#6366f1',
            'color_bg'             => $options['color_bg'] ?? '#0b1020',
            'color_panel'          => $options['color_panel'] ?? '#101626',
            'color_border'         => $options['color_border'] ?? '#1f2937',
            'color_text'           => $options['color_text'] ?? '#f9fafb',
            'color_sub'            => $options['color_sub'] ?? '#9ca3af',
    ...
    // Inline script: نبعث حدث جاهزية الإعدادات للـ JS
    $inline_script = "
    (function() {
...
function pax_sup_render_frontend_markup() {
    ...
    <div id="pax-chat" role="dialog" aria-modal="true" class="modal-mode mode-assistant">
        <div class="pax-header" id="pax-head">
            ...
            <div>
                <div class="pax-title">
                    <?php echo esc_html( $options['brand_name'] ?? 'Support' ); ?>
                </div>
            ...
            <button class="pax-iconbtn" id="pax-menu-btn" type="button" title="<?php esc_attr_e( 'Menu', 'pax-support-pro' ); ?>">
                <svg viewBox="0 0 24 24"><path d="M3 18h18v-2H3v2zm0-5h18v-2H3v2zm0-7v2h18V6H3z"/></svg>
            </button>

            <button class="pax-iconbtn" id="pax-head-more" type="button" title="<?php esc_attr_e( 'Quick Actions', 'pax-support-pro' ); ?>">
            ...
            <button class="pax-iconbtn" id="pax-close" type="button" title="<?php esc_attr_e( 'Close', 'pax-support-pro' ); ?>">
